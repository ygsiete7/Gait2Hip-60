...
NPZ_PATH = os.path.join(REPO_ROOT, "data", "Gait2Hip_MF60.npz")
...
X = data["X"].astype(np.float32)          # (N, T, D_in)
Y_raw = data["Y"].astype(np.float32)      # Raw target, unit: N
...
# Convert raw muscle force targets to body-weight-normalized training targets.
# Raw dataset unit: N; model training target unit: N/kg.
...
print("Raw Y shape (N):", Y_raw.shape)
print("Training target Y shape (N/kg):", Y.shape)
...
print("\n" + "=" * 90)
print("5-fold GroupKFold benchmark on POOL (Mamba baseline, MSE only)")
print("=" * 90)
...
cv_csv_path = os.path.join(RESULTS_DIR, "Mamba_MF60_cv_5fold_baseline_metrics.csv")
...
speed_csv_path = os.path.join(RESULTS_DIR, "Mamba_MF60_test_metrics_by_speed_mean.csv")
...
# =========================================================
ckpt_path = os.path.join(SAVE_DIR, "Mamba_MF60.pt")
...
torch.save({
    ...
    "protocol": "Subject-holdout TEST + 5-fold GroupKFold benchmark on POOL; baseline Mamba; MSE only; report subject-mean metrics; raw target unit=N; model target unit=N/kg",
